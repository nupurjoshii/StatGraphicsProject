...
btc_price_column <- btc %>%
  dplyr::select(price) %>%
  rename(Bitcoin = price)

eth_price_column <- eth %>%
  dplyr::select(price) %>%
  rename(Ethereum = price)

doge_price_column <- doge %>%
  dplyr::select(price) %>%
  rename(Dogecoin = price)

cardano_price_column <- cardano %>%
  dplyr::select(price) %>%
  rename(Cardano = price)

binancecoin_price_column <- binancecoin %>%
  dplyr::select(price) %>%
  rename(Binancecoin = price)
...
btc_pchange_column <- btc %>%
  ...
  rename(Bitcoin = pc_change)

eth_pchange_column <- eth %>%
  mutate(index = 100*price/price[1],
           pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(pc_change) %>%
  rename(Ethereum = pc_change)

doge_pchange_column <- doge %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(pc_change) %>%
  rename(Dogecoin = pc_change)

cardano_pchange_column <- cardano %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(pc_change) %>%
  rename(Cardano = pc_change)

binancecoin_pchange_column <- binancecoin %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(pc_change) %>%
  rename(Binancecoin = pc_change)
...
sandp_pchange_column <- sandp_hist_prices %>%
  ...
  rename(`S & P 500` = pc_change)

dowjones_pchange_column <- dji_hist_prices %>%
  mutate(index = 100*close/close[1],
           pc_change = 100*(close- lag(close))/lag(close)) %>%
  dplyr::select(pc_change,date) %>%
  rename(`Dow Jones` = pc_change)
...
btc_index_column <- btc %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>% 
  dplyr::select(index) %>%
  rename(Bitcoin = index)

eth_index_column <- eth %>%
  mutate(index = 100*price/price[1],
           pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(index) %>%
  rename(Ethereum = index)

doge_index_column <- doge %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(index) %>%
  rename(Dogecoin = index)

cardano_index_column <- cardano %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(index) %>%
  rename(Cardano = index)

binancecoin_index_column <- binancecoin %>%
  mutate(index = 100*price/price[1],
         pc_change = 100*(price- lag(price))/lag(price)) %>%
  dplyr::select(index) %>%
  rename(Binancecoin = index)
...
sandp_index_column <- sandp_hist_prices %>%
  ...
  rename(`S & P 500` = index)

dowjones_index_column <- dji_hist_prices %>%
  mutate(index = 100*close/close[1],
           pc_change = 100*(close- lag(close))/lag(close)) %>%
  dplyr::select(index,date) %>%
  rename(`Dow Jones` = index)
...
btc_tweets_column <- btc_tweets %>%
  ...
  rename(`Bitcoin Tweets` = pc_TweetCount)

eth_tweets_column <- eth_tweets %>%
  mutate(pc_TweetCount = 100*(TweetCount- lag(TweetCount))/lag(TweetCount)) %>% 
  dplyr::select(pc_TweetCount) %>%
  rename(`Ethereum Tweets` = pc_TweetCount)

doge_tweets_column <- doge_tweets %>%
  mutate(pc_TweetCount = 100*(TweetCount- lag(TweetCount))/lag(TweetCount)) %>% 
  dplyr::select(pc_TweetCount) %>%
  rename(`Dogecoin Tweets` = pc_TweetCount)
